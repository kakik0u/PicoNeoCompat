...
配布ZIP内の`nvenc_compat.ini`をメモ帳で開くと、NVENC設定をビルドなしで変更できます。SteamVRを終了して値を変更し、`Install.bat`を再実行してください。
...
まずは上記の組み合わせがおすすめです。現在の自動変換で使用される`P4 / HighQuality`より、エンコード負荷と遅延を抑える方向の設定です。

指定できる値：

- `Preset`: `Auto`, `P1`～`P7`（P1ほど高速、P7ほど高画質・高負荷）
- `Tuning`: `Auto`, `HighQuality`, `LowLatency`, `UltraLowLatency`, `Lossless`
- `MultiPass`: `Auto`, `Disabled`, `Quarter`, `Full`

比較する場合は、次の順で一項目ずつ試すと違いを判断しやすくなります。

1. `P2 / UltraLowLatency / Disabled`
2. `P3 / LowLatency / Disabled`
3. `P4 / LowLatency / Disabled`
4. 既定値の`Auto / Auto / Auto`

低いプリセットほど同じビットレートで画質が落ちる可能性があります。設定が読み込まれたかは`nvenc_compat.log`の`config loaded preset=...`で確認できます。無効な値は`Auto`として扱われます。
